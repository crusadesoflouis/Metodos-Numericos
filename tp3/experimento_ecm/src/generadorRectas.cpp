...
void GeneradorRectas::dame_rectas_con_cantidad(vector<Recta> &rectas, int cantidad_de_rectas, unsigned int alto, unsigned int ancho) {
    assert(cantidad_de_rectas % 4 == 0);
    uniform_int_distribution<int> x0(0, ancho - 1);  // rango [param1,param2]
    uniform_int_distribution<int> x1(1, ancho - 1);
    uniform_int_distribution<int> y1(0, ancho - 1);
